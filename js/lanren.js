...
var MouseDirection = function (element, opts) {
    ...
    var oOver = document.getElementsByClassName('cover');
    var oBack = document.getElementsByTagName('h4');
    //点击出现遮罩事件
     for(var i=0;i<oBox.length;i++){
        oBox[i].onclick = function(){
            ...
            oOver[this.index].style.display = 'block';
        }
     }
     //点击退出遮罩事件
     for(var i=0;i<oBack.length;i++){
        oBack[i].onclick = function(){
            for(var j=0;j<oBack.length;j++){
                oBack[j].index = j;
            }
            oOver[this.index].style.display = 'none';
        }
     } 
